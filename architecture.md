  ...
  - Spreadsheet/ - компоненты таблицы
     - **✅ Spreadsheet.tsx - основной компонент таблицы с ВИРТУАЛИЗАЦИЕЙ для оптимизации производительности**
      ...
      - **Обработчик скролла с дебаунсингом для плавности**
       ...
       - **НОВОЕ (DMD Cottage): скрыта нумерация строк слева от столбца A для шаблона `Отчет заселения/выселения DMD Cottage`.**
       - **НОВОЕ (DMD Cottage): для пар строк в колонке A вида "X" и следующая "X дуль" автоматически применяется жирная нижняя граница ко второй строке пары для визуального разделения групп.**
       - **НОВОЕ (DMD Cottage): авто-сортировка строк по столбцу A (начиная с 3-й строки). Пары вида "X" и "X дубль" сохраняются вместе и перемещаются как один блок при сортировке. Сортировка запускается один раз после загрузки ячеек.**
       - **НОВОЕ (DMD Cottage): автоматический расчет колонки "Доплата" (колонка 13) по формуле "Общая сумма" (колонка 11) - "Предоплата" (колонка 12). Расчет выполняется автоматически после авто-сортировки для всех строк с данными.**
       - **НОВОЕ (DMD Cottage): фиксированные ширины столбцов при автонастройке размеров: "Общая сумма" (66px), "Доплата" (79px), "Кол-во дней" (74px), "Дата выселения" (104px), "Статус дома" (94px). Автоматический перенос текста применяется ко всем ячейкам в фиксированных столбцах для предотвращения выхода текста за границы.**
       - **НОВОЕ (DMD Cottage): автоматический расчет доплаты в колонке 13 как разность между "Общей суммой" (колонка 11) и "Предоплатой" (колонка 12). Пересчет происходит автоматически при загрузке страницы, изменении даты отчета и после автосортировки строк. Доплата больше не берется из внешних источников, а рассчитывается локально в браузере.**
       - **НОВОЕ (DMD Cottage): жирное форматирование для колонок "Общая сумма" (11) и "Доплата" (13) для выделения финансовых данных. Форматирование применяется как в режиме просмотра, так и в режиме редактирования ячеек, а также сохраняется при экспорте в Excel.**
       - **ИСПРАВЛЕНО: оптимизирована функция автонастройки размеров для предотвращения бесконечных циклов запросов к серверу. Добавлены точные проверки изменений размеров с учетом погрешности, флаг для предотвращения повторных вызовов и улучшена логика применения форматирования переноса текста.**
       - **ИСПРАВЛЕНО: улучшено визуальное отображение автосортировки и расчета доплаты. Добавлено принудительное обновление компонента и виртуализации после выполнения операций. Исправлен API endpoint для обновления ячеек доплаты (404 ошибка).**
       - **ИСПРАВЛЕНО: добавлен принудительный перерендер через состояние renderKey и обновление виртуализации через setScrollTop для немедленного отображения результатов автосортировки без обновления страницы.**
       - **ИСПРАВЛЕНО: добавлена блокировка операций во время автосортировки через флаг isSortingInProgressRef для предотвращения сброса сортировки множественными вызовами пересчета доплаты. Добавлен дебаунсинг пересчета доплаты при изменении даты отчета.**
       - **Экспорт**:
