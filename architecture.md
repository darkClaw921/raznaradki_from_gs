...
Сервис работает в production на сервере с внешним nginx по адресу: 
...
- **controllers/**: Обработчики HTTP запросов
  ...
  - cellController.ts - операции с ячейками (обновление, формулы, пакетное обновление)
...
- **routes/**: API маршруты
  ...
  - cellRoutes.ts - маршруты ячеек (/api/cells, включая пакетное обновление)
...
- **components/**: React компоненты
  ...
  - Spreadsheet/ - компоненты таблицы
       - **Экспорт**:
         - Excel (XLSX) — через ExcelJS: сохраняются ширины столбцов, высоты строк, объединения ячеек, границы (включая усиленную левую границу для столбцов C и G у шаблона DMD Cottage, жирную нижнюю границу для заголовков и строк данных), заливки, шрифты, выравнивание и перенос текста. Шапка отчета объединяется (A1:B1, C1:F1, G1:Q1) и центрируется.
...
- **utils/**: Утилиты frontend
  - batchRequests.ts - утилиты для пакетного обновления ячеек (группировка, дебаунсинг, задержки)
...
### Функции webhook
- **extractWebhookInfo** - извлечение типа действия и данных из webhook, включая обработку platform_tax
- **deleteBookingFromSheet** - удаление бронирования и сдвиг строк вверх
- **addBookingToSheet** - добавление/обновление бронирования в таблицу с автоматическим расчетом колонок G и H на основе platform_tax
...
- **Пакетные запросы**: Обновления ячеек группируются в пакеты по 50 штук для снижения нагрузки на сервер
- **Backend endpoint**: `/api/cells/sheets/:sheetId/cells/batch-optimized` для оптимизированного пакетного обновления
- **Дебаунсинг**: Быстро следующие друг за другом обновления объединяются перед отправкой
- **Задержка между пакетами**: 100мс между пакетами для предотвращения перегрузки сервера
- **Применение**: Используется при изменении даты отчета, автонастройке размеров, массовых операциях вставки/форматирования

### Компоненты пакетного обновления
- **batchRequests.ts**: Утилиты для группировки запросов и дебаунсинга
- **BatchDebouncer**: Класс для накопления и отправки обновлений с задержкой
- **saveCellsBatchOptimized**: Функция в Spreadsheet для пакетной отправки обновлений
...
### Spreadsheet
- **Cell.tsx** - компонент ячейки таблицы. Отвечает за отображение и редактирование данных ячейки. Реализует специальные стили для отчетов заселения/выселения (толстые границы для колонок C и G, жирное форматирование для финансовых колонок 11 и 13 на основе шаблона).
- **Spreadsheet.tsx** - основной компонент таблицы. Управляет отображением данных, экспортом в Excel с сохранением форматирования (включая толстые границы для отчетов). Передает информацию о шаблоне в компонент Cell.
  - **calculateDoplataForReport()** - функция автоматического расчета доплаты для отчета DMD Cottage. Вычисляет доплату как разность между общей суммой (колонка 11) и предоплатой (колонка 12) для всех строк с данными. Обновляет ячейки локально и синхронизирует с сервером.
...
### Версия от 15.09.2025
- **Исправлена автонастройка размеров ячеек для пользователей**: Убрана блокировка автонастройки размеров для пользователей с правами 'read'. Теперь все пользователи с доступом к таблице могут использовать автонастройку размеров ячеек в отчетах.
- **Улучшено обновление UI после автонастройки размеров**: Добавлено принудительное обновление интерфейса после изменения размеров ячеек. Теперь изменения отображаются сразу без необходимости обновления страницы.
- **Усиленная защита от нежелательных диалогов**: 
  - Добавлена защита от случайного открытия меню границ и цветов в панели инструментов
  - Добавлены флаги `disableAutoFocus`, `disableEnforceFocus`, `disableRestoreFocus` для предотвращения автоматического фокуса
  - Введена система флагов `isUserInitiated` и `isColorUserInitiated` для контроля открытия меню только по пользовательскому действию
  - Добавлен useEffect для принудительного сброса состояния меню при инициализации компонента
  - Добавлена проверка `e.isTrusted` для блокировки программных событий клика
  - Временная блокировка взаимодействия на 2 секунды после инициализации компонента (`isInitializing`)
  - Детальное логирование для диагностики источника ложных кликов
- **Улучшена система границ в отчетах**:
  - Добавлена жирная нижняя граница для строки заголовков (2-я строка) в UI и экспорте Excel
  - Добавлены жирные нижние границы для всех строк с данными, исключая парные строки ("X дубль") и строки перед парными
  - Обновлен компонент Cell с новым пропсом `hasBottomBorder` для управления жирными границами
  - Обновлена логика экспорта Excel для корректного применения жирных границ с учетом скрытых колонок
  - Исправлена логика границ между парными строками: теперь жирная граница не отображается между строками "X" и "X дубль"
  - Улучшена визуальная структура отчетов DMD Cottage с четким разделением заголовков и данных
- **Добавлен автоматический расчет platform_tax в webhook системе**:
  - В webhook обработчике добавлена поддержка поля `platform_tax` из входящих данных бронирования
  - Колонки G (индекс 6) и H (индекс 7) теперь автоматически заполняются значением `platform_tax` с округлением до целого числа
  - Добавлена обработка случаев, когда `platform_tax` равен null - в таких случаях используется значение 0
  - Обновлено логирование для отслеживания исходного и округленного значений `platform_tax`
  - Функция `extractWebhookInfo` теперь извлекает и передает `platform_tax` в структуре данных бронирования
