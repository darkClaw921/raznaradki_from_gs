...
│   │   │   └── sheetTemplateController.ts  # ✅ ИСПРАВЛЕН: Контроллер шаблонов таблиц [НОВОЕ]
│   │   ├── models/            # Модели базы данных (Sequelize)
│   │   │   └── SheetTemplate.ts  # Модель шаблонов таблиц [НОВОЕ]
│   │   ├── routes/            # Маршруты API
│   │   │   └── sheetTemplateRoutes.ts  # ✅ ИСПРАВЛЕНЫ: Маршруты для шаблонов [НОВОЕ]
...
│   │   └── app.ts            # ✅ ИСПРАВЛЕН: Основной файл приложения (обновлен с поддержкой шаблонов)
│   ├── package.json
│   ├── tsconfig.json
│   └── Dockerfile
...
│   │   │   └── Dashboard.tsx  # ✅ ИСПРАВЛЕН: Обновлен с поддержкой шаблонов [РАСШИРЕНО]
│   │   ├── hooks/            # Custom hooks
...
### Backend (/backend)
- **app.ts**: ✅ ИСПРАВЛЕН - Точка входа, настройка Express сервера, middleware, Socket.io, инициализация админа
- **controllers/**: Обработчики HTTP запросов
  ...
  - **sheetTemplateController.ts** - **✅ ИСПРАВЛЕН - работа с шаблонами таблиц (получение списка, создание таблиц из шаблонов)** [НОВОЕ]
...
- **routes/**: API маршруты
  ...
  - **sheetTemplateRoutes.ts** - **✅ ИСПРАВЛЕНЫ - маршруты шаблонов (/api/templates)** [НОВОЕ]
...
- **pages/**: Страницы приложения
  ...
  - Dashboard.tsx - **✅ ИСПРАВЛЕНА - главная страница со списком таблиц и новым диалогом создания с поддержкой шаблонов** [ЗНАЧИТЕЛЬНО РАСШИРЕНО]
...
### 2. Управление таблицами [ЗНАЧИТЕЛЬНО РАСШИРЕНО И ИСПРАВЛЕНО]
- Создание, редактирование, удаление таблиц
- **✅ СИСТЕМА ШАБЛОНОВ ТАБЛИЦ - ПОЛНОСТЬЮ РАБОТАЮЩАЯ:**
  - **✅ ИСПРАВЛЕНЫ ВСЕ ПРОБЛЕМЫ API endpoints (/api/templates/create)**
  - **✅ ИСПРАВЛЕНА логика активации кнопки создания из шаблона**
  - **✅ ИСПРАВЛЕНЫ маршруты и middleware в backend (sheetTemplateRoutes.ts)**
  - **✅ ПОЛНОСТЬЮ ПРОТЕСТИРОВАНО через Playwright - создание таблиц работает корректно**
  ...
  - **✅ СИСТЕМА СВЯЗАННЫХ ТАБЛИЦ для автоматической синхронизации:**
  ...
  - **✅ ФУНКЦИОНАЛ ВЫБОРА ИСХОДНОЙ ТАБЛИЦЫ:**
  ...
  - **Готовые шаблоны для гостиничного бизнеса DMD Cottage:**
    ...
    - **"Отчет заселения/выселения DMD Cottage"** - ежедневный отчет о заселениях и выселениях (16 колонок с разделением на секции выселения и заселения) **+ АВТОМАТИЧЕСКАЯ СВЯЗЬ С ЖУРНАЛОМ**
...
- **✅ ИСПРАВЛЕНО: Автоматическая инициализация базы данных** [27.01.2025]
  - **SQL ошибка исправлена**: экранированы зарезервированные слова `row`/`column` в таблицах
  - **При первом запуске автоматически создаются**:
    - ✅ Все таблицы (roles, users, permissions, sheets, cells, cell_history, sheet_templates)
    - ✅ 3 роли: admin, editor, user с полными разрешениями
    - ✅ Администратор admin@dmdcottage.com (пароль: admin123456)
    - ✅ 2 готовых шаблона DMD Cottage (Журнал заселения и Отчет заселения/выселения)
  - **Процесс**: schema.sql применяется через docker-entrypoint-initdb.d при старте MySQL
  - **Команда запуска**: `docker-compose up -d` - создает полностью рабочую систему
- **✅ ИСПРАВЛЕНО: Кодировка русского текста** [27.01.2025]
  - **Конфигурация MySQL**: создан файл `mysql/my.cnf` с настройками UTF-8
  - **Docker монтирование**: конфигурация подключается в docker-compose.yml
  - **Кодировка соединения**: добавлена команда `SET NAMES utf8mb4` в schema.sql
  - **Пароль администратора**: обновлен хеш для правильной авторизации (admin123456)
  - **✅ ПРОТЕСТИРОВАНО**: русские шаблоны, интерфейс отображаются корректно
