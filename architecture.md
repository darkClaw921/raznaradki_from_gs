  ...
  - Spreadsheet/ - компоненты таблицы
     - **✅ Spreadsheet.tsx - основной компонент таблицы с ВИРТУАЛИЗАЦИЕЙ для оптимизации производительности**
      ...
      - **Обработчик скролла с дебаунсингом для плавности**
       ...
       - **НОВОЕ: Пакетное обновление ячеек - при массовых операциях (изменение даты отчета, автонастройка) запросы группируются в пакеты по 50 штук с задержкой 100мс между пакетами для оптимизации производительности.**
       - **Экспорт**:
...
- **Spreadsheet.tsx** - основной компонент таблицы. Управляет отображением данных, экспортом в Excel с сохранением форматирования (включая толстые границы для отчетов). Передает информацию о шаблоне в компонент Cell.
  - **calculateDoplataForDMDCottage** - автоматический расчет доплаты для шаблона DMD Cottage по формуле: Доплата = Общая сумма - Предоплата. Выполняется после автосортировки.
  - **Автосортировка DMD Cottage** - useEffect с правильными зависимостями для стабильной работы функций сортировки и расчета доплаты. Включает сохранение отсортированных ячеек на сервер. Автоматически перезапускается при изменении даты отчета без необходимости обновления страницы.
  - **Защита от перезагрузки** - предотвращение перезагрузки ячеек после автосортировки для сохранения отсортированных данных. Защита работает во всех местах где происходит перезагрузка данных (useEffect загрузки ячеек, handleAutoResize, обновление настроек).
